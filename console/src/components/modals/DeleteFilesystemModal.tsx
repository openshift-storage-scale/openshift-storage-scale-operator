...
import { k8sDelete, k8sGet, k8sPatch, useK8sModel } from '@openshift-console/dynamic-plugin-sdk';
...
const DeleteFilesystemModal: React.FC<DeleteFilesystemModalProps> = ({ onClose, filesystem }) => {
  ...
  const onDelete = async () => {
    ...
    try {
      ...
      const disks = [...filesystem.spec?.local?.pools.reduce((disks, pool) =>  {
      ...
      if (disks.length) {
        ...
        const diskDeletions = await Promise.allSettled(disks.map((d) => 
          k8sDelete({
        ...
        const failedDiskRemovals = diskDeletions.some((d) => d.status === 'rejected')
            ...
            [
              ...
              ...diskDeletions.reduce((acc, curr, idx) => {
                if (curr.status === 'rejected') {
                  const description = curr.reason instanceof Error ? curr.reason.message : (curr.reason as string)
  ...
  return (
    <Modal
        ...
        actions={[
          <Button key="confirm" variant="danger" onClick={onDelete} isLoading={isDeleting} isDisabled={isDeleting}>
            {t("Delete")}
          </Button>,
        ...
        <Stack hasGutter>
          <StackItem isFilled>
            <Trans t={t} ns="public">
              ...
              <strong>
                {{ resourceName: filesystem.metadata?.name }}
              </strong>{' '}from namespace{' '}
              <strong>
              {{ resourceNs: filesystem.metadata?.namespace }}
              </strong>
              ?
            </Trans>
          </StackItem>
          {errors?.length && (<StackItem>
            <Alert isInline variant='danger' title={t("An error occurred while deleting resources.")}>
              <Stack>
                {errors.map((e, index) => <StackItem key={index}>{e}</StackItem>)}
              </Stack>
            </Alert>
          </StackItem>)}
        </Stack>
...
const TableDeleteFilesystemModal = () => {
  const { filesystem, setFileSystem } = React.useContext(FileSystemTableContext);
  if (!filesystem) {
    return null;
  }
  return <DeleteFilesystemModal filesystem={filesystem} onClose={() => setFileSystem(undefined)} />
}
